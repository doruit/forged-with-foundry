...
python3 "${VALIDATOR}" --contract "${COMPLETE_CONTRACT}" --control PRI-PRE-002 >/dev/null
python3 "${VALIDATOR}" --contract "${INCOMPLETE_CONTRACT}" --control PRI-PRE-002 >/dev/null
python3 "${VALIDATOR}" --contract "${COMPLETE_CONTRACT}" --control PRI-PRE-002 --enforce >/dev/null
set +e
python3 "${VALIDATOR}" --contract "${INCOMPLETE_CONTRACT}" --control PRI-PRE-002 --enforce >/dev/null 2>&1
exit_code=$?
set -e
if [[ "${exit_code}" -eq 0 ]]; then
  echo "error: --enforce should fail on a missing purpose reference" >&2
  exit 1
fi
...
echo "PRI-PRE-002 templates, scripts, and governance contracts are valid. Boundary-case"
echo "coverage lives in tests/test_pri_pre_002_governance_contract.py and"
