# Use PRI-001 from Copilot Studio: A2A and MCP
...
Copilot Studio (or any A2A/MCP-capable) agent use the exact same PRI-001
...
- Exact, production-grade trace-context propagation across systems (Copilot
  Studio → A2A/MCP tool input) still requires tenant-specific validation;
...
| | A2A | MCP |
|---|---|---|
| Choose it when | The agent should delegate an entire task to a specialized, already-governed agent | The agent needs PII redaction as one governed capability among its other tools |
| What actually runs | A full A2A task handled by the PRI-001-governed Foundry agent | The `redact_text` / `redact_document` MCP tools |
| What it protects | The receiving agent, for whatever delegation actually reaches this endpoint | Only that tool call — and only if the agent chooses to call it |
| What triggers the gate | Every inbound A2A task is gated automatically; there is no opt-out inside this adapter | The calling agent decides, per turn, whether to invoke the tool |
| Correlation IDs | Derived from the A2A task/context IDs when the SDK provides them, or freshly generated otherwise | The caller must supply `agent_id`/`run_id`/`trace_id` as explicit tool arguments |
| On block or failure | The A2A task itself is marked `FAILED` with an explicit message; the wrapped Foundry agent is never called | The tool call raises an error back to the calling agent, which decides what happens next |
| Who produces the final answer | This adapter's own governed Foundry agent (`GovernedAgent.run()`), returned as the A2A task result | The calling agent (e.g. Copilot Studio's own model), using the tool's result |
| Bypass risk this design cannot close | The calling agent may choose not to delegate to this endpoint at all | The calling agent may simply not call the tool for a given turn |

Copilot Studio can invoke *either* pattern in two ways, and this choice is
symmetric — it is not a difference between A2A and MCP: **dynamically**,
where generative orchestration decides at runtime whether to delegate to
the A2A agent or call the MCP tool, or **deterministically**, by delegating
to the A2A agent or calling the MCP tool explicitly from a topic when the
routing must be known in advance. That choice belongs to how the Copilot
Studio agent is authored, not to the protocol itself. What genuinely
differs between A2A and MCP is only what happens *after* Copilot Studio
decides to make the call — summarized in the table above — not whether
Copilot Studio is more or less likely to make it. This repository's live
walkthroughs use dynamic (generative) invocation for both; wiring a topic
to call either one deterministically is a Copilot Studio authoring
exercise, not a change to either server in this folder, and has not been
demonstrated here.
...
A2A and MCP are two **independent** integration patterns. An integration
...
- **Purple node** (`pii.control.evaluated`) proves that PRI-001 was actually
  evaluated for that run — emitted only by the A2A or MCP adapter.
...
Unlike MCP below, this gate is mandatory for every task this endpoint
...
Unlike A2A above, the MCP tool is only ever invoked if the calling agent's
...
> running server — see "Run the A2A or MCP server standalone" below and the
> [Live Copilot Studio A2A walkthrough](#live-copilot-studio-a2a-walkthrough).
...
<details>
<summary>Run the A2A or MCP server standalone</summary>

Use these if you want to connect a real A2A or MCP client (including a live
...
The A2A and MCP servers run different ASGI apps (`mcp_server:create_app` vs
...
2. `pii.control.evaluated` — the control attestation, emitted only by the
   A2A or MCP adapter when PRI-001 actually ran.
...
A2A/MCP endpoint calls alone is not enough: without the independent run
...
would provide an independent run inventory in a live tenant; the A2A/MCP
...
live Copilot Studio tenant into A2A/MCP tool inputs has not been validated
...
A2A/MCP call, sharing the same `run_id` and trace context — kept outside
...
- Both A2A and MCP reuse the exact same PRI-001 policy, detection, and
  redaction logic — the decision is not reimplemented per protocol.
- Both the A2A and MCP routes work end to end against real, deployed,
...
- When Copilot Studio is the entry point, the original prompt has already
  reached Copilot Studio before A2A or MCP selection happens. This demo
...
- Add an authentication boundary in front of the A2A server (its
  `Authentication: None` setting has no built-in bearer-token option in
  this repo yet — treat this as a genuine gap to close before any non-demo
  use, not just a config toggle to flip).
...
Running the A2A/MCP servers and the compliance evaluator locally creates no
...
If you also deployed the A2A and/or MCP servers via
