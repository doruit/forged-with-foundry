#!/usr/bin/env bash
# Deploy only the additional Azure resources owned by PRI-001.
set -euo pipefail
...
DEPLOYMENT_NAME="pri-001-pii-exposure"
...
update_env AZURE_LANGUAGE_ENDPOINT "$(output_value languageEndpoint)"
update_env PII_STORAGE_BLOB_ENDPOINT "$(output_value piiStorageBlobEndpoint)"
update_env PII_SOURCE_CONTAINER "$(output_value piiSourceContainerName)"
update_env PII_TARGET_CONTAINER "$(output_value piiTargetContainerName)"

echo "✅ PRI-001 resources deployed incrementally; updated ${CONTROL_ENV}."
