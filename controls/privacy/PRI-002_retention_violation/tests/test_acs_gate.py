...
from src.pri_002.acs_gate import (
    ApprovalTicket,
    GuardedDeletePolicyDispatcher,
    denied_resolver,
    get_control,
    resolver_for,
)
...
async def _run_guarded_delete(resolver) -> object:
    control = get_control()
...
def test_approved_ticket_allows_the_guarded_delete() -> None:
    ticket = ApprovalTicket(approved=True, issued_at=datetime.now(UTC))

    value = asyncio.run(_run_guarded_delete(resolver_for(ticket)))
...
def test_rejected_ticket_is_denied() -> None:
    ...
    try:
        asyncio.run(_run_guarded_delete(resolver_for(ticket)))
        raised = False
    except AgentControlBlocked:
        raised = True
...
def test_replayed_ticket_is_denied_on_second_use() -> None:
...
def test_approved_ticket_resolver_allows_both_pre_and_post_tool_call() -> None:
    ticket = ApprovalTicket(approved=True, issued_at=datetime.now(UTC))
