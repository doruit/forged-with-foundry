#!/usr/bin/env bash
# Deploy only the additional Azure resources owned by PRI-004.
set -euo pipefail
...
DEPLOYMENT_NAME="pri-004-personal-data-in-logs"
...
update_env PRI004_WORKSPACE_CUSTOMER_ID "$(output_value workspaceCustomerId)"
update_env PRI004_TABLE_NAME "$(output_value tableName)"
update_env PRI004_DCR_IMMUTABLE_ID "$(output_value dataCollectionRuleImmutableId)"
update_env PRI004_DCR_ENDPOINT "$(output_value dataCollectionRuleLogsIngestionEndpoint)"
update_env PRI004_STREAM_NAME "$(output_value streamName)"

echo "✅ PRI-004 resources deployed incrementally; updated ${CONTROL_ENV}."
