#!/usr/bin/env bash
# Deploy only the Azure Policy definition and assignment owned by PRI-PRE-001.
set -euo pipefail
...
require PRIPRE001_POLICY_ASSIGNMENT_NAME
...
PRIPRE001_POLICY_DEFINITION_ID="$(az deployment sub create \
  --name pri-pre-001-policy \
  --location "${AZURE_LOCATION}" \
  --template-file "${SCRIPT_DIR}/policy-definition.bicep" \
  --parameters policyDefinitionName="${PRIPRE001_POLICY_DEFINITION_NAME}" \
...
PRIPRE001_POLICY_ASSIGNMENT_ID="$(az deployment group create \
  --name pri-pre-001-assignment \
  --resource-group "${AZURE_RESOURCE_GROUP}" \
  --mode Incremental \
  --template-file "${SCRIPT_DIR}/main.bicep" \
  --parameters policyDefinitionId="${PRIPRE001_POLICY_DEFINITION_ID}" \
               policyAssignmentName="${PRIPRE001_POLICY_ASSIGNMENT_NAME}" \
  --query properties.outputs.policyAssignmentId.value \
  --output tsv)"
update_env PRIPRE001_POLICY_ASSIGNMENT_ID "${PRIPRE001_POLICY_ASSIGNMENT_ID}"

echo "PRI-PRE-001 policy deployed. Allow several minutes for assignment propagation."
