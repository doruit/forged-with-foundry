...
async def _run_guarded_classify(resolver) -> object:
    control = get_control()
...
def test_approved_ticket_allows_the_guarded_classify() -> None:
    ticket = ApprovalTicket(approved=True, issued_at=datetime.now(UTC))

    value = asyncio.run(_run_guarded_classify(resolver_for(ticket)))
...
def test_missing_ticket_is_denied_not_auto_approved() -> None:
    ...
    try:
        asyncio.run(_run_guarded_classify(denied_resolver))
        raised = False
    except AgentControlBlocked:
        raised = True
...
def test_rejected_ticket_is_denied() -> None:
    ...
    try:
        asyncio.run(_run_guarded_classify(resolver_for(ticket)))
        raised = False
    except AgentControlBlocked:
        raised = True
