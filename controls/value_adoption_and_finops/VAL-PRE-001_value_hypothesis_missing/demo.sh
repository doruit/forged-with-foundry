...
echo "1/2 Assessing a workload with an incomplete value hypothesis..."
assess_contract "${INCOMPLETE_CONTRACT}"
echo "Validating a go-live request with valueHypothesisStatus=${CONTROL_STATUS}..."
if az deployment group validate \
  ...
  --parameters demoResourceName="${VALPRE001_DEMO_RESOURCE_NAME}" \
               valueHypothesisStatus="${CONTROL_STATUS}" \
...
echo "2/2 Assessing a workload with a complete, measurable value hypothesis..."
assess_contract "${COMPLETE_CONTRACT}"
echo "Validating the same request with valueHypothesisStatus=${CONTROL_STATUS}..."
if ! az deployment group validate \
