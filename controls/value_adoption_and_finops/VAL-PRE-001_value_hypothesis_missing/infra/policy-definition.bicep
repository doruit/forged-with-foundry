...
resource valueHypothesisGatePolicy 'Microsoft.Authorization/policyDefinitions@2021-06-01' = {
  ...
  properties: {
    ...
    description: 'Denies a tagged go-live request unless valueHypothesisStatus=approved and a non-empty businessCaseId is present. The full structured hypothesis (metric, target, direction, baseline, owner) is assessed by scripts/validate_value_hypothesis.py against agent.yaml before these two tags are set; Azure Policy only re-checks the reduced result. Illustrative teaching policy; not production governance guidance.'
    policyType: 'Custom'
    mode: 'Indexed'
    metadata: {
