#!/usr/bin/env bash
# Deploy only the Azure Policy definition and assignment owned by VAL-PRE-002.
set -euo pipefail
...
require VALPRE002_POLICY_ASSIGNMENT_NAME
...
VALPRE002_POLICY_DEFINITION_ID="$(az deployment sub create \
  --name val-pre-002-policy \
  --location "${AZURE_LOCATION}" \
  --template-file "${SCRIPT_DIR}/policy-definition.bicep" \
  --parameters policyDefinitionName="${VALPRE002_POLICY_DEFINITION_NAME}" \
  --query properties.outputs.policyDefinitionId.value \
  --output tsv)"
export VALPRE002_POLICY_DEFINITION_ID
update_env VALPRE002_POLICY_DEFINITION_ID "${VALPRE002_POLICY_DEFINITION_ID}"

echo "2/2 Assigning the policy to the selected resource group..."
VALPRE002_POLICY_ASSIGNMENT_ID="$(az deployment group create \
  --name val-pre-002-assignment \
  --resource-group "${AZURE_RESOURCE_GROUP}" \
  --mode Incremental \
  --template-file "${SCRIPT_DIR}/main.bicep" \
  --parameters policyDefinitionId="${VALPRE002_POLICY_DEFINITION_ID}" \
               policyAssignmentName="${VALPRE002_POLICY_ASSIGNMENT_NAME}" \
  --query properties.outputs.policyAssignmentId.value \
  --output tsv)"
update_env VALPRE002_POLICY_ASSIGNMENT_ID "${VALPRE002_POLICY_ASSIGNMENT_ID}"

echo "VAL-PRE-002 policy deployed. Allow several minutes for assignment propagation."
