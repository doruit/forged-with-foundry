...
def create_event_logger(credential: DefaultAzureCredential) -> tuple[logging.Logger, LoggerProvider]:
    ...
    provider = LoggerProvider(resource=Resource.create({"service.name": "VAL-001"}))
    exporter = AzureMonitorLogExporter(
        connection_string=os.environ["VAL001_APPLICATIONINSIGHTS_CONNECTION_STRING"],
        credential=credential,
        disable_offline_storage=True,
    )
    provider.add_log_record_processor(SimpleLogRecordProcessor(exporter))
    logger = logging.getLogger("fwf.val001.outcomes")
