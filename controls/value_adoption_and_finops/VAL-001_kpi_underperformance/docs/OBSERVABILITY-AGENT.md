...
The live query was also investigated through the Observability Agent. The
captures in step 2 show the exact prompt and the agent's result for the same
run. They demonstrate the exploratory agent experience, but the deterministic
evaluator and its evidence record remain authoritative.
...
let perPeriod =
   ...
   | summarize total = count(),
         ...
         invalidEvents = countif(not(verificationValid or outcomeValid
            or item_count == 1))
      by period
...
Open the Observability Agent beside the Logs view and use this prompt:

```text
Analyze TicketTriaged events in AppEvents from the last 24 hours for run ID
880c44b5-a049-4a8a-a9b3-be377de954e3, control ID VAL-001, and metric name
tier1_ticket_deflection_rate. Parse the event payload from the JSON string in
Properties.event.

Include only events where verified equals "True" or "true".

For each period:
- Count all matching events and those with outcome deflected.
- Calculate the deflection percentage using event counts, not ItemCount
   weighting, rounded to two decimal places.
- Compare the percentage against a threshold of 28%, calculated as 80% of the
   35% target.
- Label the result below_threshold if the percentage is strictly below 28%;
   otherwise, label it at_or_above_threshold.

Return review_required only if exactly two periods are present and both are
below the threshold. Otherwise, return no_review_required.

Show the per-period results, the final decision, and the KQL query used.
```

Replace the run ID when investigating another synthetic run.

![VAL-001 investigation prompt in the Azure Observability Agent](../media/azure-observability-agent-prompt.png)

The prompt capture records the exact run, control, metric, threshold, and
decision rule supplied to the Observability Agent. It contains no ticket
content or personal identifiers.

![Observability Agent result for the VAL-001 underperformance run](../media/azure-observability-agent-result.png)

The result capture shows five events and one deflection in each period, a
20.00% rate for both periods, and the suggested `review_required` outcome. It
also shows the KQL generated by the agent. This narrative is an investigation
aid only and does not replace the evaluator's completeness, integrity, or
fail-closed checks.
...
| where observed > 1 or array_length(outcomes) > 1
   or array_length(verification) > 1
