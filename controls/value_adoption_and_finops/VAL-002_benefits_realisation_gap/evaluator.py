"""Evaluate one complete synthetic milestone window against the validated
VAL-PRE-001 entry.

Unlike VAL-001's rolling two-period check, this evaluator pools every ticket
across all six required periods into a single cumulative rate before
comparing it to the milestone threshold -- it answers "has this business
case realized at least half its target after six periods?", not "did either
of the last two periods dip?". Pooling (sum of deflected / sum of total)
is used deliberately instead of averaging per-period percentages, so periods
with different ticket volumes cannot distort the result.
"""
...
def contract_target(path: Path) -> dict:
    ...
    return {
        ...
        "business_case_id": evidence["businessCaseId"],
        "target": target,
        "threshold": target * Decimal("0.5"),
    }
...
def measure(manifest: dict, rows: list[dict]) -> list[dict]:
    ...
    if len(periods) != REQUIRED_PERIODS:
        raise CannotEvaluate("require_six_periods")
    ...
    if sequence != list(range(sequence[0], sequence[0] + REQUIRED_PERIODS)):
    ...
    for row in rows:
        ...
        if (event.get("run_id") != run_id or event.get("agent_id") != AGENT_ID
                or event.get("control_id") != "VAL-002" or event.get("metric_name") != METRIC
    ...
    for earlier, later in zip(ordered, ordered[1:]):
        if max(event_times[earlier]) > min(event_times[later]):
...
def evaluate(path: Path, manifest: dict, rows: list[dict], *, query_complete: bool) -> dict:
    ...
    record = {
        "control_id": "VAL-002", "control_version": VERSION, "policy_version": VERSION,
        "evidence_version": VERSION, "agent_id": AGENT_ID, "metric_name": METRIC,
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "correlation_id": None, "contract": None, "periods": [], "window_periods": REQUIRED_PERIODS,
        "decision": "cannot_evaluate", "reason": "invalid_input",
        "accountable_role": "Business Owner", "owner": None, "business_case_id": None,
        "target_percent": None, "threshold_percent": None, "pooled_percent": None,
    ...
    try:
        ...
        record.update(owner=target.pop("owner"), business_case_id=target.pop("business_case_id"),
                  target_percent=float(target_value), threshold_percent=float(threshold_value),
                  contract=target)
        if not query_complete:
            raise CannotEvaluate("telemetry_query_unavailable_or_partial")
        measurements = measure(manifest, rows)
        total_deflected = sum(period["deflected"] for period in measurements)
        total_tickets = sum(period["total"] for period in measurements)
        pooled_percent = float(Decimal(total_deflected) * 100 / total_tickets)
        underperforming = Decimal(total_deflected) * 100 < threshold_value * total_tickets
        record.update(periods=measurements, pooled_percent=pooled_percent,
                      decision="reassess_required" if underperforming else "no_reassessment_required",
                      reason="pooled_rate_strictly_below_half_target" if underperforming
                      else "cumulative_shortfall_not_observed")
