...
def evidence(tmp_path):
    ...
    save(path, {"decision": "reassess_required", "correlation_id": "synthetic-run",
                "owner": "synthetic-owner@contoso.example", "business_case_id": "BC-2026-014",
                "target_percent": 35, "threshold_percent": 17.5, "pooled_percent": 16.67,
                "window_periods": 6, "periods": [
                    {"sequence": number, "total": 6, "deflected": 1, "percent": 16.67}
...
def test_given_failure_notification_when_measurement_recovers_then_history_is_retained():
    ...
    current = {"decision": "reassess_required", "notification": {"status": "not_requested"}}
...
def test_given_reassess_required_card_then_red_attention_signal_is_visible(evidence):
    ...
    assert "Value hypothesis reassessment required" in status_text
...
def test_given_healthy_card_then_green_positive_signal_is_visible(evidence):
    record = json.loads(evidence.read_text())
    record["decision"] = "no_reassessment_required"
    payload = card(record)
    ...
    assert "Benefits realisation on track" in status_text
    assert "at or above half of target" in status_text


def test_given_reassess_required_card_then_business_case_is_visible(evidence):
    payload = card(json.loads(evidence.read_text()))
    status_text = json.dumps(payload, ensure_ascii=False)

    assert "BC-2026-014" in status_text
    assert "Pooled rate" in status_text
