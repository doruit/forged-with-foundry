"""Run, evaluate, and notify for the isolated VAL-002 synthetic demonstration."""
...
RUNS = CONTROL / ".azure/val002/runs"
...
def run_scenario(scenario: str) -> str:
    ...
    Six periods per run (not VAL-001's two) so the evaluator has a full
    milestone window to pool. `total` is deliberately small (6 tickets per
    period, 36 per run) to keep a live demo run's invocation count close to
    VAL-001's, while still producing a clear pooled signal either side of
    the 50%-of-target threshold.
    """
    run_id = str(uuid4())
    folder = RUNS / run_id
    counts, total = {
        "milestone_shortfall": ((1, 1, 1, 1, 1, 1), 6),
        "milestone_met": ((2, 2, 2, 2, 2, 2), 6),
    }[scenario]
...
def card(record: dict) -> dict:
    ...
    status = {
        "reassess_required": {
            ...
            "title": "VAL-002: Value hypothesis reassessment required",
            "message": "Cumulative results across all six periods remain below half of target. Business Owner reassessment requested.",
        },
        "cannot_evaluate": {
            "icon": "⚠", "color": "warning", "container": "warning",
            "title": "VAL-002: KPI measurement unavailable",
            "message": "Restore the measurement path before treating this milestone as healthy.",
        },
        "no_reassessment_required": {
            "icon": "●", "color": "good", "container": "good",
            "title": "VAL-002: Benefits realisation on track",
            "message": "Cumulative results across all six periods are at or above half of target. No reassessment trigger was observed.",
        },
    }.get(decision, {
        "icon": "⚠", "color": "warning", "container": "warning",
        "title": "VAL-002: Decision requires attention",
    ...
    if cannot_evaluate:
        ...
        facts.extend([
            ...
            {"title": "Business case", "value": record["business_case_id"]},
            {"title": "Target", "value": f"{record['target_percent']}%"},
            {"title": "Threshold (50% of target)", "value": f"{record['threshold_percent']}%"},
            {"title": "Pooled rate (6 periods)", "value": f"{record['pooled_percent']}%"},
        ])
...
def notification_route(record: dict, config: dict) -> tuple[str, str] | None:
    ...
    Deliberately reuses VAL-001's webhook settings (VAL001_TEAMS_WEBHOOK_URL /
    VAL001_GOVERNANCE_TEAMS_WEBHOOK_URL) rather than declaring VAL002_*
    variables: this control posts to the same Business Owner and AI
    Governance Operations Teams Workflows VAL-001 already configured, per an
    explicit decision recorded in this control's ASSESSMENT.md, so its cards
    appear in the same Workflows chat as VAL-001's rather than requiring a
    second Power Automate flow to be authored.
    """
    if record["decision"] == "reassess_required":
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--scenario", choices=["milestone_shortfall", "milestone_met"],
                        default="milestone_shortfall")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "evaluate":
            ...
            with path.with_suffix(".lock").open("a") as lock:
                ...
                if previous and previous["decision"] != "cannot_evaluate":
                    ...
                    rows, complete = query_outcomes(config.get("VAL002_WORKSPACE_ID", ""), run_id)
        ...
        print(json.dumps({"run_id": run_id, "decision": record["decision"],
                          ...
                          "pooled_percent": record.get("pooled_percent"),
