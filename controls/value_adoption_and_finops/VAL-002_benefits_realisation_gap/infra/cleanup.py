"""Remove only verified VAL-002 Azure resources and hosted sessions.
...
def validate_resources(subscription: str, group: str, insights: dict, workspace: dict) -> None:
    ...
    for resource, resource_type, name_prefix in (
        (insights, "Microsoft.Insights/components", "appi-val002-"),
        (workspace, "Microsoft.OperationalInsights/workspaces", "log-val002-"),
    ):
        ...
        if (resource["id"].lower() != expected or resource["type"].lower() != resource_type.lower()
                ...
                or resource.get("tags", {}).get("control-id") != "VAL-002"
                or resource.get("tags", {}).get("purpose") != "synthetic-benefits-realisation-demo"):
...
def main() -> int:
    ...
    try:
        outputs = json.loads(command([
            ...
            "--resource-group", args.resource_group, "--name", "val002-monitor",
